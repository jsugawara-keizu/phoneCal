...
import labelEventDetail from "@salesforce/label/c.PhoneCal_EventDetail";
import labelOpenRecord from "@salesforce/label/c.PhoneCal_OpenRecord";
...
export default class PhoneCalendar extends NavigationMixin(LightningElement) {
  ...
  label = {
    ...
    eventDetail: labelEventDetail,
    openRecord: labelOpenRecord
  };

  // ── Event detail panel (compact layout) ─────────────────────────────────────
  @track showEventDetail = false;
  @track selectedEventId = null;
  @track eventDetailRows = [];
  compactFieldNames = [];
  ...
  // ── Wire: compact layout field definitions (Event object, no recordId needed) ─
        ...
        for (const item of row.layoutItems) {
          for (const comp of item.layoutComponents) {
            if (comp.componentType === "Field" && comp.apiName) {
              defs.push({ apiName: comp.apiName, label: item.label });
            }
          }
        }
      }
    }
    this._compactFieldDefs = defs;
    this.compactFieldNames = defs.map((f) => `Event.${f.apiName}`);
  }

  // ── Wire: selected event record data (fires when selectedEventId is set) ─────

  @wire(getRecord, {
    recordId: "$selectedEventId",
    optionalFields: "$compactFieldNames"
  })
  wiredEventRecord({ data }) {
    if (!data || !this._compactFieldDefs.length) {
      return;
    }
    this.eventDetailRows = this._compactFieldDefs
      .map((f) => ({
        key: f.apiName,
        label: f.label,
        value:
          getFieldDisplayValue(data, `Event.${f.apiName}`) ??
          getFieldValue(data, `Event.${f.apiName}`)
      }))
  ...
  get hasEventDetailRows() {
    return this.eventDetailRows.length > 0;
  }
  ...
  handleOpenRecord() {
    const recordId = this.selectedEventId;
    this.handleCloseEventDetail();
